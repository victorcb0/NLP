...
parse(Neterm, As, W0, Wn) :-
    wf(Neterm, _, W0, _), !,
    wf(Neterm, As, W0, Wn);
    fail_wf(Neterm, W0), !, fail.
...
parse(Nterm, As, [W0 | W1], Wn) :-
    ...
    call(P),
    assertz(wf(Nterm, As, [W0 | W1], Wn)).

parse(Neterm, _As, W0, _Wn) :-
    (wf(Neterm, _, W0, _);
    assertz(fail_wf(Neterm, W0))), !, fail.
...
test_doc :- tell('2.left-corner/2.left-corner-wf.doc'),
    testp(P, A),
    write([prop, =, P]), nl,
    write([arb, =, A]), told.
...
test_all :-
    tell('2.left-corner/2.left-corner-wf-1000.doc'),
